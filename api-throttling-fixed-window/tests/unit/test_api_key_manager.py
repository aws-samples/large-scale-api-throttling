...
def test_api_key_manager():
    ...
    table.put_item(Item={"user_id": "test_user", "api_key": "test_key", "window_duration": 42, "window_requests":43})

    manager = UserApiKeyManager(api_key_table)

    # Test key that was inserted
    api_key = manager.get_api_key_throttling("test_user")
    assert api_key["api_key"] == "test_key"
    assert api_key["window_duration"] == 42
    assert api_key["window_requests"] == 43

    # Test getting a user that is not in the table
    with pytest.raises(KeyError):
        manager.get_api_key_throttling("test_user2")
